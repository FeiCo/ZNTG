...
- (instancetype)initWithSetting:(WSFSlideTitlesViewSetting *)setting
{
    ...
    if (self) {
        ...
        for (int i = 0; i < titlesCount; ++i) {
            ...
            // 第一个按钮
            if (setting.initialButtonNumber == i) {
                [self titlesBtnClick:titlesButton action:YES];
                ...
                if (!setting.lineHidden) {
                    ...
                    if (setting.lineWidth) {
                        ...
                        if (setting.isFullDisplay == YES) {
                            lineView.frame = CGRectMake(0, 0, setting.lineWidth, setting.frame.size.height);
                        ...
                        lineView =
                        [[UIView alloc] initWithFrame:CGRectMake(0, 0, titlesButton.titleLabel.frame.size.width,setting.lineHeight)];
                        
                        if (setting.isFullDisplay == YES) {
                            lineView.frame = CGRectMake(0, 0, titlesButton.titleLabel.frame.size.width, setting.frame.size.height);
                        }
                    }
                    lineView.backgroundColor = setting.lineColor;
                    lineView.center = CGPointMake((setting.frame.size.width / setting.titlesArr.count) / 2, setting.frame.size.height / 2);
                    
                    if (setting.isFullDisplay == NO) {
                        
                lineView.center = CGPointMake((setting.frame.size.width / setting.titlesArr.count) / 2,setting.frame.size.height - setting.lineBottomSpace);
                    }
...
- (void)selectButtonAtIndex:(NSUInteger)index action:(BOOL)clickOrNot
{
    if (index < self.buttonArr.count) {
        [self titlesBtnClick:self.buttonArr[index] action:clickOrNot];
    }
}
...
-(void) clickButton:(UIButton *) button
{
  ...
  [self titlesBtnClick:button action:YES];
    
}

- (void)titlesBtnClick:(UIButton *)button action:(BOOL)clickOrNot
{
    {
        // 已点击按钮
        ...
        [UIView animateWithDuration:self.setting.animateDuration
                         animations:^{
                             if (self.setting.lineWidth) {
                                 ...
                                 if (self.setting.isFullDisplay == YES) {
                                     self.lineView.frame = CGRectMake(self.lineView.frame.origin.x,
                                                                      self.lineView.frame.origin.y, self.setting.lineWidth, self.setting.frame.size.height);
                                 }
                             }
                             else {
                                 self.lineView.frame
                                 = CGRectMake(self.lineView.frame.origin.x, self.lineView.frame.origin.y,button.titleLabel.frame.size.width, self.setting.lineHeight);
                                 if (self.setting.isFullDisplay == YES) {
                                     self.lineView.frame = CGRectMake(self.lineView.frame.origin.x,
                                                                      self.lineView.frame.origin.y, button.titleLabel.frame.size.width, self.setting.frame.size.height);
                             }
                             ...
                             self.lineView.center = CGPointMake(button.center.x, self.setting.frame.size.height / 2);
                             
                             if (self.setting.isFullDisplay == NO) {
                                 
                                 self.lineView.center = CGPointMake(button.center.x,self.setting.frame.size.height - self.setting.lineBottomSpace);
...
-(int)initialButtonNumber
{

    if (!_initialButtonNumber) {
        _initialButtonNumber = 0;
    }
    return _initialButtonNumber;
