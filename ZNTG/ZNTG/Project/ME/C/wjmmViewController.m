...
- (void)onClickVerifyCodeBtn:(JKCountDownButton *)sender{
    ...
    if([phoneString length] <= 0 || !isPhoneNumber){
        ...
        NSDictionary *dictionary = @{@"telephone":phoneString};
        [MSNetRequest requestMethodsWithPOST:dictionary url:kForgetPSWOne successBlock:^(id responseObject) {
...
-(void)onClickNextBtn:(UIButton *)sender {
    ...
    else
    {
        ...
        NSDictionary *dictionary = @{@"code":verifyCodeString,@"firstpassword":passWordString,@"secondpassword":againpassWordString,@"telephone":phoneString};
        [MSNetRequest requestMethodsWithPOST:dictionary url:kForgetPSWTwo successBlock:^(id responseObject) {
            ...
            NSNumber *type = [jsonDictionary objectForKey:@"type"];
             if (type.integerValue == 1){
                ...
                [self showHUDWithStr:[jsonDictionary objectForKey:@"msg"]];
            }
        } failureBlock:^(NSString *errorDescribe) {
            [self showHUDWithStr:@"网络不给力！"];
        }];
